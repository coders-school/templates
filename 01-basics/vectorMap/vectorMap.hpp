...
public:
  using iterator =
      typename std::vector<KeyType>::iterator; // dependent type name
  using key_type = KeyType;
  using mapped_type = ValueType;
  using value_type = std::pair<const KeyType, ValueType>;

  static constexpr bool is_int_key =
      std::is_same_v<KeyType, int>; // type_traits #2
...
public:
  ...
  VectorMap() { std::cout << "bool specialization used\n"; }
...
template <class Map> inline constexpr bool is_int_key_v = Map::is_int_key;
